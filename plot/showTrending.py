...
def plot_publish_hours(df):
    fig = plt.figure()
    #plt.
    output = sb.distplot(df['publish_hour'], color="maroon", kde=False)
    output.set_xlabel("Publish Hour", fontsize=12)
    output.set_ylabel("Number of trending videos", fontsize=12)
    return mpld3.fig_to_html(fig)


def plot_tags(df):
    fig = plt.figure()
    output = sb.distplot(df['tags_count'], color="teal", kde=False)
    output.set_xlabel("Number of tags", fontsize=12)
    output.set_ylabel("Number of trending videos", fontsize=12)
    return mpld3.fig_to_html(fig)
